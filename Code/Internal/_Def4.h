...
public:
	...
	void LookAt(const glm::vec3& targetPosition) {
		...
		float pitch = glm::atan(direction.y, glm::length(glm::vec2(direction.x, direction.z)));

		wRotation.y = glm::degrees(yaw); 
		wRotation.x = glm::degrees(-pitch); 
		wRotation.z = 0.0f;		// The object should not rotate around the Z-axis in a typical LookAt (roll is ignored)
